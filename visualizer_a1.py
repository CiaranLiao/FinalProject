...
class Visualizer(imgui_window.ImguiWindow):
    ...
    def __init__(self, capture_dir=None):
        super().__init__(title='', window_width=3840, window_height=2160)
    ...
    def draw_frame(self):
        ...
        # self.pane_w = 0
        # self.button_w = 0
        # self.label_w = round(self.font_size * 0)
        ...
        print(dx, dy)
        if dx<=5.0:
            self.latent_widget.drag(8.0, 0)

        if dy>=5.8 and dy<=6.2:
            self.layer_widget.cur_layer="output"
            print("done")
        elif dy>=5.5 and dy<=6.5:
            self.layer_widget.cur_layer="L14_512_3"
            print("soon")
        elif dy!=0 and dy<5.2:
            rand=random.randint(-3,3)
            self.layer_widget.channel_change(rand)
        elif dy!=0 and dy>6.8:
            self.layer_widget.cur_layer = "L13_512_64"
            rand=random.randint(-3,3)
            self.layer_widget.channel_change(rand)
        else:
            self.layer_widget.cur_layer = "L13_512_64"

        # if dy>14 and channels==70:
        #     self.layer_widget.cur_layer = "output"
        #     channels=0
        # elif dy>10 and channels>50:
        #     self.layer_widget.cur_layer = "L14_512_3"
        #     channels=70
        # elif dy>2:
        #     self.layer_widget.channel_change(1)
        #     if channels>=61:
        #         channels=61
        #     else:
        #         channels += 1
        # else:
        #     self.layer_widget.cur_layer = "L13_512_64"
        #     self.layer_widget.channel_change(-3)
        #     if channels<=0:
        #         channels=0
        #     else:
        #         channels -= 3
        ...
        # imgui.begin('##control_pane', closable=True)
        ...
        max_w = self.content_width
        max_h = self.content_height
        pos = np.array([max_w/2-512-10, max_h / 2])
        if 'image' in self.result:
            if self._tex_img is not self.result.image:
                self._tex_img = self.result.image
                if self._tex_obj is None or not self._tex_obj.is_compatible(image=self._tex_img):
